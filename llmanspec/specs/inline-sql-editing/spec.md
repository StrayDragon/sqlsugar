...
requirements[8]{req_id,title,statement}:
  ...
  "R-ISE-005",临时文件清理修复,系统 MUST 通过 onDidCloseTextDocument hook 实现可配置的临时文件自动清理
  "R-ISE-006",SQL 内容检测,系统 SHALL 通过关键词启发式检测内容是否为 SQL 并允许用户确认
  "R-ISE-007",测试覆盖,系统 MUST 达到 80% 以上的单元测试覆盖率
  "R-ISE-008",JS/TS 引号升级,系统 MUST 在 JS/TS 中当内容包含换行时自动升级为模板字面量
