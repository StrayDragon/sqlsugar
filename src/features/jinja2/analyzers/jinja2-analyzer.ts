 ...
 */
export class Jinja2Analyzer implements Analyzer {
  readonly name = 'jinja2';
  readonly priority = 5; // Highest priority - Jinja2 first
  ...
  private extractVariablesFromExpression(expr: string): string[] {
    ...
    // Extract identifiers from the main expression
    IDENTIFIER_REGEX.lastIndex = 0;
    while ((match = IDENTIFIER_REGEX.exec(mainExpr)) !== null) {
      const varName = match[1];
      // Skip function calls (has parenthesis right after)
