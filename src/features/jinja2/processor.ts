...
import type { TemplateFingerprint } from '../ui/types/memory';
import { templateFingerprinter } from '../ui/utils/template-fingerprinter';
...
export class Jinja2NunjucksProcessor {
  ...
  /**
   * Generate template fingerprint for memory persistence
   *
   * @param template - The Jinja2 template content
   * @param options - Optional fingerprinting options
   * @returns Template fingerprint structure
   */
  public generateTemplateFingerprint(
    template: string,
    options?: { includeContentHash?: boolean; sensitivity?: 'low' | 'medium' | 'high' }
  ): TemplateFingerprint {
    try {
      const startTime = Date.now();
      const fingerprint = templateFingerprinter.generateFingerprint(template, options);
      const duration = Date.now() - startTime;

      memoryLogger.fingerprint('debug', 'Template fingerprint generated by processor', fingerprint.structureHash, duration);

      return fingerprint;
    } catch (error) {
      memoryLogger.error('Failed to generate template fingerprint in processor', error as Error, {
        operation: 'generateTemplateFingerprint',
        templateLength: template.length
      });

      // Return minimal fingerprint on error
      return {
        structureHash: this.generateBasicHash(template),
        variableNames: this.extractVariableNames(template),
        variableCount: 0,
        contentHash: this.generateBasicHash(template),
        templateLength: template.length,
        created: Date.now(),
        lastSeen: Date.now()
      };
  ...
  /**
   * Find similar templates for memory lookup
   *
   * @param template - The current template
   * @param knownFingerprints - Array of known template fingerprints
   * @param similarityThreshold - Minimum similarity threshold (0-1)
   * @returns Array of similar fingerprints
   */
  public findSimilarTemplates(
    template: string,
    knownFingerprints: TemplateFingerprint[],
    similarityThreshold: number = 0.8
  ): TemplateFingerprint[] {
    try {
      const currentFingerprint = this.generateTemplateFingerprint(template);
      return templateFingerprinter.findSimilarFingerprints(
        currentFingerprint,
        knownFingerprints,
        similarityThreshold
      );
    } catch (error) {
      memoryLogger.error('Failed to find similar templates in processor', error as Error, {
        operation: 'findSimilarTemplates',
        knownFingerprintsCount: knownFingerprints.length
      });
      return [];
    }
  }
  ...
  /**
   * Extract variable names from template for memory purposes
   *
  ...
  public extractVariableNames(template: string): string[] {
    ...
    try {
      // Enhanced variable extraction using multiple patterns
      const patterns = [
        // Standard variable pattern: {{ variable }}
        /\{\{\s*([^}]+)\s*\}\}/g,
        // Variable with filters: {{ variable | filter }}
        /\{\{\s*([^|}]+)(?:\s*\|\s*[^}]+)?\s*\}\}/g,
      ...
      patterns.forEach(pattern => {
        ...
        while ((match = pattern.exec(template)) !== null) {
          ...
          if (variableExpression) {
            ...
            const cleanName = this.cleanVariableName(variableExpression);
            if (cleanName && !variableNames.includes(cleanName)) {
              variableNames.push(cleanName);
            }
          }
        }
      });
      ...
      memoryLogger.error('Failed to extract variable names in processor', error as Error, {
        operation: 'extractVariableNames',
        templateLength: template.length
      });
      return [];
    }
  ...
  /**
   * Process template with enhanced memory support
   *
   * @param template - The Jinja2 template
   * @param context - Template context variables
   * @param fingerprint - Optional pre-generated template fingerprint
   * @returns Rendered template with metadata
   */
  public renderTemplateWithMemory(
    template: string,
    context: Record<string, unknown>,
    fingerprint?: TemplateFingerprint
  ): { rendered: string; fingerprint: TemplateFingerprint; variables: string[] } {
    const startTime = Date.now();

    try {
      // Generate fingerprint if not provided
      const templateFingerprint = fingerprint || this.generateTemplateFingerprint(template);

      // Extract variable names for memory purposes
      ...
      const duration = Date.now() - startTime;
      memoryLogger.performance('renderTemplateWithMemory', duration, {
        templateLength: template.length,
        variableCount: variableNames.length,
        fingerprint: templateFingerprint.structureHash
      });
      ...
      const duration = Date.now() - startTime;
      memoryLogger.error('Failed to render template with memory', error as Error, {
        operation: 'renderTemplateWithMemory',
        duration,
        templateLength: template.length
      });
      ...
      return {
        ...
        fingerprint: fingerprint || this.generateMinimalFingerprint(template),
        variables: this.extractVariableNames(template)
      };
    }
  }

  /**
   * Clean variable name from expression
   */
  private cleanVariableName(expression: string): string | null {
    if (!expression) return null;

    // Remove filters and modifiers
    const cleaned = expression
      .split('|')[0] // Remove filters
      .replace(/\s+/g, ' ') // Normalize whitespace
      .trim();

    // Handle dot notation (object.property)
    if (cleaned.includes('.')) {
      // Return the full expression for now, could be enhanced later
      return cleaned;
    }

    // Handle array notation (array[index])
  ...
  /**
   * Generate basic hash for fallback fingerprinting
   */
  private generateBasicHash(template: string): string {
    let hash = 0;
  ...
  /**
   * Generate minimal fingerprint for error fallback
   */
  private generateMinimalFingerprint(template: string): TemplateFingerprint {
    return {
      structureHash: this.generateBasicHash(template),
      variableNames: this.extractVariableNames(template),
      variableCount: 0,
