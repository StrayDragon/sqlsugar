...
/**
 * Convert Jinja2Variable to UnifiedVariable
 */
function toUnifiedVariable(v: Jinja2Variable): UnifiedVariable {
  return {
    name: v.name,
    type: v.type,
    sourceType: 'jinja2',
...
export class VariableEditor extends LitElement {
  ...
  @property({ type: Array })
  accessor variables: Jinja2Variable[] = [];

  @property({ type: Array })
  accessor parameters: ExtractedParameter[] = [];

  @property({ type: Object })
  accessor values: Record<string, Jinja2VariableValue> = {};
  ...
  private getUnifiedVariables(): UnifiedVariable[] {
    ...
    const paramVars = this.parameters
      .filter((p) => p.type !== 'jinja2') // Exclude jinja2 from parameters (already in variables)
