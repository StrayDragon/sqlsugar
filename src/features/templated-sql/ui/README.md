# Templated SQL Editor - 下一代可视化模板编辑器

## 概述

Templated SQL Editor 是可视化模板编辑器的完全重写版本，专注于**直接模板交互**。用户现在可以直接点击模板中高亮的变量进行内联编辑，而不是在单独的面板中编辑变量。
...
```
src/templated-sql-editor/
├── components/
│   ├── templated-sql-editor.ts      # 主编辑器组件
│   ├── template-highlighter.ts   # 模板解析和高亮
│   ├── variable-popover.ts       # 内联变量编辑器
│   └── templated-sql-preview.ts        # 增强预览组件
├── utils/
...
├── config/
│   └── templated-sql-editor-config.ts       # 配置管理
├── styles/
│   ├── animations.ts             # 动画定义
...
├── index.ts                      # 公共导出
└── test/
    └── templated-sql-editor-test.html  # 交互式测试页面
```
...
Templated SQL Editor 包含在 SQLSugar 扩展中，通过命令直接打开（无需额外开关）。可选行为见下方配置选项。

### 使用

1. **打开命令面板**（`Ctrl+Shift+P` 或 `Cmd+Shift+P`）
2. **运行** `SQLSugar: Copy To Templated SQL (Editor)`
3. **编辑**模板，直接点击高亮的变量
4. **使用键盘快捷键**进行高效导航
...
```json
{
  "sqlsugar.templatedSqlEditor.popoverPlacement": "auto",
  "sqlsugar.templatedSqlEditor.highlightStyle": "background",
  "sqlsugar.templatedSqlEditor.autoPreview": true,
  "sqlsugar.templatedSqlEditor.keyboardNavigation": true,
  "sqlsugar.templatedSqlEditor.animationsEnabled": true,
  "sqlsugar.templatedSqlEditor.showSuggestions": true,
  "sqlsugar.templatedSqlEditor.autoFocusFirst": false
}
```
...
Templated SQL Editor 自动适配您的 VSCode 主题：
...
在浏览器中打开测试页面 `templated-sql-editor-test.html` 以交互式测试 Templated SQL Editor。
