...
export class VariablePopover extends LitElement {
  ...
  @property({ attribute: false }) accessor currentValue: TemplateVariableValue = undefined;
  ...
  @state() accessor localValue: TemplateVariableValue = undefined;
  @state() accessor localType: TemplateVariableType = 'string';
  ...
  show(variable: EnhancedVariable, currentValue: TemplateVariableValue) {
  ...
  private handleTypeChange(event: CustomEvent) {
    const { value } = event.detail;
    this.localType = value as TemplateVariableType;
    this.localValue = getDefaultValueForType(this.localType);
  }

  private handleSuggestionClick(suggestion: TemplateVariableValue) {
          ...
          ${quickSuggestions.length > 0 ? html`
            <div class="form-group">
              ...
              <div class="quick-suggestions">
                ${quickSuggestions.map((suggestion: TemplateVariableValue) => html`
                  <button
  ...
  private formatSuggestion(suggestion: TemplateVariableValue): string {
    const formatted = this.formatValue(suggestion);
    return formatted.length > 15 ? formatted.substring(0, 12) + '...' : formatted;
  }

  private isValueActive(suggestion: TemplateVariableValue): boolean {
