...
export class SqlHighlighter {
   ...
   */
  highlightSQL(sql: string, variables: Record<string, TemplateVariableValue> = {}): HighlightResult {
    if (!sql) {
      return {
        html: '',
        variables: [],
   ...
   */
  private detectVariablePositions(sql: string, variables: Record<string, TemplateVariableValue>): VariablePosition[] {
   ...
   */
  private fallbackHighlight(sql: string, variables: Record<string, TemplateVariableValue>): HighlightResult {
    const keywords = [
   ...
   */
  private formatValueForSQL(value: TemplateVariableValue): string {
   ...
   */
  private formatValueForDisplay(value: TemplateVariableValue): string {
    if (value == null) return 'NULL';
