...
import { createTemplateVariable } from './variable-utils.js';
import {
  JINJA2_REGEX,
  JINJA2_KEYWORDS,
  extractVariableFromExpression,
} from '../../../../shared/template-patterns.js';
...
function inferVariableType(variableName: string, context: VariableContext, filters: string[] = []): TemplateVariable['type'] {
  ...
  // 判成 email/url，进而渲染成 <input type=email/url> 的“页面组件”并预填
  // example.com（webview 里点开会触发跳转）。改为统一当作 string。
