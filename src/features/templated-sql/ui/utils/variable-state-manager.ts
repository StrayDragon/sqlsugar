...
export class VariableStateManager {
   ...
   */
  private getDefaultValue(variable: EnhancedVariable): TemplateVariableValue {
   ...
   */
  getValue(variableName: string): TemplateVariableValue | undefined {
   ...
   */
  getAllValues(): Record<string, TemplateVariableValue> {
   ...
   */
  private convertValueToType(value: TemplateVariableValue, targetType: string): TemplateVariableValue {
   ...
   */
  private getDefaultValueForType(type: string): TemplateVariableValue {
    switch (type) {
      case 'string': return '';
   ...
   */
  getSuggestions(variableName: string, limit: number = 5): TemplateVariableValue[] {
    const state = this.states.get(variableName);
    const variable = this.variables.get(variableName);
    ...
    const uniqueValues = new Set<TemplateVariableValue>();
   ...
   */
  private getContextualSuggestions(variable: EnhancedVariable): TemplateVariableValue[] {
    const name = variable.name.toLowerCase();
   ...
   */
  private validateValue(value: TemplateVariableValue, type: string): boolean {
    return validateValue(value, type as TemplateVariableType) === null;
  }
