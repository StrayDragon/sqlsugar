...
export class TemplatedSqlWebviewEditor {
   ...
   * 静态方法：提取模板变量（供外部调用）
   */
  public static extractVariables(template: string): TemplateVariable[] {
    try {
      const handler = TemplatedSqlHandler.getInstance();
  ...
  private show(template: string, variables: TemplateVariable[], title: string): void {
  ...
  private updateContent(template: string, _variables: TemplateVariable[]): void {
    if (!this.panel) {
      return;
    }
