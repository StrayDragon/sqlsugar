...
export class Jinja2NunjucksHandler {
     ...
     * 处理Jinja2模板 - 主要入口点
     */
    public static async handleCopyJinja2Template(mode: 'quick' | 'wizard' | 'webview' | 'defaults' = 'quick'): Promise<boolean> {
        try {
            ...
            vscode.window.showErrorMessage(`Failed to process Jinja2 template: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    ...
    private async processTemplate(mode: string): Promise<boolean> {
        ...
        // 检测占位符类型
        const placeholderDetection = SQLAlchemyPlaceholderProcessor.detectPlaceholderTypes(selectedText);
        ...
        if (variables.length === 0 && !placeholderDetection.hasSQLAlchemy) {
            vscode.window.showInformationMessage('No Jinja2 variables or SQLAlchemy placeholders found in selected template.', { modal: false });
            return false;
        }
    ...
    private async handleSQLAlchemyOnly(template: string, sqlalchemyVars: string[]): Promise<boolean> {
        try {
            ...
            vscode.window.showErrorMessage(`SQLAlchemy mode failed: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    ...
    private async handleQuickMode(
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Quick mode failed: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    ...
    private async handleWizardMode(
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Wizard mode failed: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    ...
    private async handleWebviewMode(
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Webview mode failed: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    ...
    private async handleDefaultsMode(
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Defaults mode failed: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    ...
    private async copyToClipboard(
        ...
        try {
            ...
            if (userValues && usedDefaults > 0) {
                const customValues = Object.keys(userValues).filter(key =>
                    JSON.stringify(userValues[key]) !== JSON.stringify(variables.find(v => v.name === key)?.defaultValue)
                );
