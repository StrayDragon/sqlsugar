...
export class Jinja2VariableInput {
     ...
     * 显示变量配置界面
     */
    public static async configureVariables(variables: Jinja2Variable[]): Promise<Record<string, any> | undefined> {
     ...
     * 智能模式 - 自动推断类型但允许修改
     */
    public static async smartConfigure(variables: Jinja2Variable[]): Promise<Record<string, any> | undefined> {
        const context: Record<string, any> = {};
        ...
        for (const variable of variables) {
            ...
            switch (choice.value) {
                ...
                case 'change_type':
                    ...
                    value = await this.inputValue(variable.name, newType, this.getDefaultValueForType(newType));
                    if (value === undefined) {
                        return undefined;
                    }
                    break;
