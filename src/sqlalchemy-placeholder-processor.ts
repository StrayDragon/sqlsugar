    ...
    public static validateMixedPlaceholders(sql: string): {
        ...
        if (detection.hasJinja2) {
            try {
                ...
                errors.push(`Jinja2 syntax error: ${error instanceof Error ? error.message : String(error)}`);
            }
        }

        // 检查SQLAlchemy占位符
        if (detection.hasSQLAlchemy) {
            const invalidPlaceholders = detection.sqlalchemyVars.filter(variable =>
        ...
        if (conflictingVars.length > 0) {
            warnings.push(`Variable names used in both Jinja2 and SQLAlchemy: ${conflictingVars.join(', ')}`);
        }
